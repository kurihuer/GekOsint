...
def render_missing_keys(data):
    keys = []
    if isinstance(data, dict):
        keys = data.get("missing_keys") or []
    if not keys:
        return ""
    keys = [k for k in keys if k]
    if not keys:
        return ""
    txt = render_section("API KEYS (OPCIONAL)")
    txt += "<b>Faltan:</b> " + ", ".join(f"<code>{k}</code>" for k in keys) + "\n"
    txt += "<i>Configúralas en variables de entorno para mejores resultados.</i>\n"
    return txt

def format_dns_result(data):
    if not data or "error" in data: return "Error analizando el dominio."
    ...
    if data.get("a_records"):
        txt += render_section("REGISTROS A (IPs)")
        for ip in data["a_records"][:5]:
            txt += f"📍 <code>{ip}</code>\n"
    ...
    if whois:
        ...
        if whois.get("status"):
            txt += f"🛡️ <b>Estado:</b> {whois['status'][:50]}...\n"
    ...
    txt += (
        ...
        f"<a href='https://securitytrails.com/domain/{d}'>SecTrails</a> | "
...
def format_phone_result(data):
    ...
    if cgeo and "error" not in cgeo:
        txt += render_section("GEOLOCALIZACIÓN (Carrier - Aproximada)")
        txt += f"<b>IP (carrier):</b> <code>{cgeo.get('ip', data.get('carrier_ip', 'N/A'))}</code>\n"
        txt += f"<b>País:</b> {cgeo.get('country', 'N/A')}\n"
        txt += f"<b>Ciudad:</b> {cgeo.get('city', 'N/A')}\n"
        txt += f"<b>ISP:</b> {cgeo.get('isp', 'N/A')}\n"
        txt += f"<b>Zona:</b> {cgeo.get('timezone', 'N/A')}\n"
        if cgeo.get("map_url"):
            txt += f"<a href='{cgeo['map_url']}'>🗺️ Ver en Maps</a>\n"
        txt += "\n<i>Nota: esto NO es la IP del objetivo; es una referencia del operador.</i>\n"

    ipi = data.get("ip_intel")
    if isinstance(ipi, dict) and ipi:
        txt += render_section("IP INTELLIGENCE")
        if "error" in ipi:
            target = data.get("ip_intel_target")
            if target:
                txt += f"<b>Entrada:</b> <code>{target}</code>\n"
            txt += f"<b>Estado:</b> ❌ {ipi.get('error')}\n"
        else:
            target = data.get("ip_intel_target")
            ip_val = ipi.get("ip", "N/A")
            if target and str(target).strip() and str(target).strip() != str(ip_val).strip():
                txt += f"<b>Entrada:</b> <code>{target}</code>\n"
            risk_bar = _risk_bar(ipi.get("risk_score", 0))
            txt += f"<b>IP:</b> <code>{ip_val}</code>\n"
            txt += f"<b>Ubicacion:</b> {ipi.get('city', 'N/A')}, {ipi.get('country', 'N/A')}\n"
            txt += f"<b>ISP:</b> {ipi.get('isp', 'N/A')}\n"
            txt += f"<b>Riesgo:</b> {risk_bar} {ipi.get('risk', 'N/A')} ({ipi.get('risk_score', 0)}/100)\n"
            if ipi.get("proxy"):
                txt += f"<b>Proxy/VPN:</b> {ipi.get('proxy')}\n"
            if ipi.get("map_url"):
                txt += f"<a href='{ipi['map_url']}'>Abrir en Google Maps</a>\n"
...
def format_whatsapp_result(data):
    ...
    if caller_name:
        ...
        txt += render_section("IDENTIDAD")
        txt += "<b>Nombre:</b> No disponible\n"
        txt += "<i>Nota: WhatsApp no expone el nombre públicamente; depende de fuentes externas (ej. Truecaller/Numbway).</i>\n"
    ...
    if photo:
        ...
        txt += "<i>Nota: en servidores cloud suele bloquearse el scraping o no hay acceso público a la foto.</i>\n"
...
def format_geoloc_coords(data):
    txt = render_header("GEOLOCALIZACIÓN")
    txt += f"📍 <b>Coordenadas:</b>\n"
    txt += f"   Latitud: <code>{data['lat']}</code>\n"
    txt += f"   Longitud: <code>{data['lon']}</code>\n"
    txt += f"   🗺️ <a href='{data['map_url']}'>Ver en Google Maps</a>\n"
    txt += f"\n<i>Tipo: {data.get('type', 'coordenadas')}</i>"
    return txt

def format_geoloc_ip(data):
    if "error" in data:
        return f"❌ {data['error']}"
    ...
    txt += f"🏢 <b>Org:</b> {data.get('org', 'N/A')}\n"
    txt += f"🔢 <b>AS:</b> {data.get('as', 'N/A')}\n"
    txt += render_section("CARACTERÍSTICAS")
    txt += f"📱 <b>Móvil:</b> {'Sí' if data.get('mobile') else 'No'}\n"
    txt += f"🏢 <b>Hosting:</b> {'Sí' if data.get('hosting') else 'No'}\n"
    txt += f"🔒 <b>Proxy/VPN:</b> {'Sí' if data.get('proxy') else 'No'}\n"
    txt += f"\n🗺️ <a href='{data.get('map_url', '')}'>Ver en Maps</a>\n"
    return txt

def format_geoloc_webrtc(data):
...
def format_wifi_scan(data):
    ...
    txt = render_header("ESCÁNER WiFi")
    ...
    txt += f"\n<i>Fuente: {data.get('source', 'desconocido')}</i>"
    return txt
